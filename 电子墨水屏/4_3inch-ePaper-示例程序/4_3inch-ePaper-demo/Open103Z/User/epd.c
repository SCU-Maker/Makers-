...
static void _putchars(const unsigned char * ptr, int n)
{
	...
	for(i = 0; i < n; i++)
	{
		USART_SendData(USART1, (unsigned char)ptr[i]); 
...
void epd_init(void)
{
  GPIO_InitTypeDef GPIO_InitStructure;
	...
	GPIO_InitStructure.GPIO_Pin = EPD_PORTA_PIN_OUT; 
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOA, &GPIO_InitStructure);	
	
	GPIO_ResetBits(EPD_WAKEUP_PORT, EPD_WAKEUP_PIN);   
...
void epd_reset(void)
{
  ...
  GPIO_ResetBits(EPD_RESET_PORT, EPD_RESET_PIN); 
	...
	CM3_SysTickDelayMS(3000);                                                       //复位后强制延时 3秒
}
/*******************************************************************************
* Function Name  : void epd_init(void)
* Description    : 
...
void epd_wakeup(void)
{
	...
	GPIO_SetBits(EPD_WAKEUP_PORT, EPD_WAKEUP_PIN);
  ...
  GPIO_ResetBits(EPD_WAKEUP_PORT, EPD_WAKEUP_PIN); 
	...
	CM3_SysTickDelayMS(10);                                                       //唤醒后强制延时 10ms	
}
